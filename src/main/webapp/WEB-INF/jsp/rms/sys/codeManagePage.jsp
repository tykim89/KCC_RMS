...
<script src="http://code.jquery.com/jquery-1.6.4.min.js"></script>
 ...
 $(document).ready(
			function(){
				...
				var codeDetailModiResult = '${codeDetailModiResult}';
				var codeDetailDeleteResult = '${codeDetailDeleteResult}';
				...
				if(codeDetailModiResult !="")
				{
					if(codeDetailModiResult != 0)
					{
						alert("코드 수정 성공.");					
					}else if(codeDetailModiResult == 0)
					{
						alert("코드 수정 실패.");
					}
				}
				if(codeDetailDeleteResult !="")
				{
					if(codeDetailDeleteResult != 0)
					{
						alert("코드 삭제 성공.");					
					}else if(codeDetailDeleteResult == 0)
					{
						alert("코드 삭제 실패.");
					}
				}
				...
				var detailCodeDeleteResult ='${groupDeleteResult}';
				...
				{
					if(groupDeleteResult != 0)
					{
						alert("코드그룹 삭제 성공.");					
					}else if(groupDeleteResult == 0)
					{
						alert("코드그룹 삭제 실패.");
					}
				}
 ...
 function checkAll(allBtnId, checkBtnId)
	{
		   	...
		   	else
			{
		   		if(confirm("삭제 하시겠습니까?"))
		   		{
		   			$("#checkedValue").val(checkedValue);
		   		 	$.post("<c:url value='/rms/sys/checkUnderItem.do'/>",$("#detailCodeForm").serialize(), function(xml)
			                  {						
		   		 		alert($(xml).find('check').text());
						 		/* 	if($(xml).find('check').text()=="")
						 			{				 				
	   ...
	   function codeDetailDeleteSubmit()
	   {
		   var checkedValue=getCheckedValue('detailCodeChecked');
		   	...
		   	else
			{
		   		...
		   		{
		   			$("#group_code1").val($("#group_code").val());
		   			$("#detailCheckedValue").val(checkedValue);		   			
		   			$.post("<c:url value='/rms/sys/deleteCodeDetail.do'/>", $("#detailCodeForm").serialize(), function(html){
  			   			$("#detailCodeListForm").html(html);
			...
			<article>
                ...
                <div class="contents">
                    ...
                    <section class="sec_thirdX2" style="padding-right:1%">
                        ...
                        <input type="hidden" name="detailCheckedValue" id="detailCheckedValue" value=""/>
                        <input type="hidden" name="group_code1" id="group_code1" value=""/>
                        <input type="hidden" id ="version_detail" name="version" value="0"/>
                        <div class="listType1">
