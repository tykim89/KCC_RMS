...
function unUseProcess()
{
	...
	var rightTarget = getCheckedValue("useProcessCheckBtn");
   ...
   else
   {
      ...
      {
    	  result = removeRightTarget("useProcessCheckBtn");
          $("#modifyTarget").val(result);
...
function useProcess()
{
	...
	//현재 체크한 절차 항목
	var leftTarget = getCheckedValue("unUseProcessCheckBtn");
	
	//누적된 절차 항목 가져오기
	var modifyTarget = $("#modifyTarget").val();
	$("#modifyTarget").val(modifyTarget + leftTarget);
...
function removeRightTarget(checkBtnId) {
	...
	var modifyTarget = $("#modifyTarget").val();
	var modifyTargetList = modifyTarget.split('|');
	var rightTargetList = [];
	var itemStr = "";
	...
	$.each(rightTargetList, function(rightIndex, rightItem) {
		$.each(modifyTargetList, function(addIndex, addItem) {
			if (modifyTargetList[addIndex] == rightTargetList[rightIndex]) {
				modifyTargetList.remove(addIndex);
			}
		});
	});

	$.each(modifyTargetList, function(index, item) {
		if (modifyTargetList[index] != "") {
			itemStr += item + "|"
		}
	});
...
//등록 체크
function insertRecProcess()
{
	//필수체크
	if($("#rec_prog_name").val()=="" || $("#rec_prog_memo").val() == "")
	{
	...
	else
	{
		if(confirm('<spring:message code="common.regist.msg" />'))
		{
			...
			$("#recProcessInsert").attr("action", "<c:url value='/rms/recProcess/insertRecProcess.do'/>").submit();
		}
	}
}
                                    ...
                                    <div class="tree_wrap" style="height: 250px;">
										<div id="useProcess" class="listType4">
											<table>
												...
												<thead>
													<tr>
														<th scope="col"><input name="useProcessAllCheckBtn" id="useProcessAllCheckBtn" class="" type="checkbox" /></th>
														<th scope="col" class="last">선택</th>
													</tr>
												</thead>
												<tbody>
				                    	...
				                    	<div class="paging_right">
				                        	...
				                        	<a class="" href="#" onclick="insertRecProcess()"><span class="ico_v"></span>등 록</a>
				                        </span>
